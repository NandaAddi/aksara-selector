  ...
  return (
    <div className="fixed inset-0 bg-black -z-10 overflow-hidden">
      {BG_IMAGES.map((img, i) => (
        <div 
          key={i}
          className={`absolute inset-0 transition-opacity duration-[2000ms] ease-in-out ${i === index ? 'opacity-40' : 'opacity-0'}`}
        >
...
const LoginPanel = ({ onLogin }) => {
  ...
  const handleManualSubmit = async (e) => {
    ...
    try {
        if (isRegister) {
            await axios.post(`${BASE_API}/api/auth/register`, formData);
            alert("Registrasi Berhasil! Silakan Login.");
            setIsRegister(false); setFormData({ name: '', email: '', password: '' });
        } else {
            const res = await axios.post(`${BASE_API}/api/auth/login`, { email: formData.email, password: formData.password });
            onLogin(res.data);
        }
      ...
      <div className="w-full max-w-md relative z-10 animate-fade-up">
        <div className="glass-card rounded-[32px] p-8 md:p-10 relative overflow-hidden">
          ...
          <form onSubmit={handleManualSubmit} className="space-y-4 mb-8">
            {isRegister && <input name="name" type="text" placeholder="Full Name" value={formData.name} onChange={handleChange} required className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:bg-white/10 transition-all text-sm" />}
            <input name="email" type="email" placeholder="Email Address" value={formData.email} onChange={handleChange} required className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:bg-white/10 transition-all text-sm" />
            <input name="password" type="password" placeholder="Password" value={formData.password} onChange={handleChange} required className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:bg-white/10 transition-all text-sm" />
          ...
          <button onClick={() => googleLogin()} disabled={isLoading} className="w-full bg-white/5 border border-white/20 hover:bg-white/10 text-white py-3.5 rounded-xl font-bold text-xs uppercase tracking-wide flex items-center justify-center gap-3 transition-all mb-6">
             Google Access
          </button>
...
const AdminPanel = ({ user, onPreviewClient, onLogout }) => {
  ...
  return (
    <div className="min-h-screen text-white p-4 md:p-8 font-sans animate-fade-up">
      ...
      <style>{`.input-field { width: 100%; bg: transparent; border: 1px solid rgba(255,255,255,0.1); border-radius: 0.75rem; padding: 1rem; color: white; outline: none; background: rgba(255,255,255,0.05); transition: all; } .input-field:focus { background: rgba(255,255,255,0.1); border-color: rgba(255,255,255,0.3); } .btn-icon { padding: 0.75rem; background: rgba(255,255,255,0.1); border-radius: 0.5rem; color: white; transition: all; } .btn-icon:hover { background: rgba(255,255,255,0.2); }`}</style>
    </div>
  );
};

/* ==================================================================================
   3. CLIENT GALLERY (MOBILE FRIENDLY + LIGHTBOX)
   ================================================================================== */
const ClientGallery = ({ config, onCloseSimulation }) => {
  ...
  return (
    <div className="min-h-screen text-white pb-24">
      {/* Navbar Responsive */}
      <nav className="fixed top-0 w-full z-40 glass-card border-b-0 border-white/5 shadow-2xl">
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-16 md:h-20 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
                ...
                <button onClick={sendToWa} className="bg-white text-black px-4 md:px-6 py-2 md:py-3 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-gray-200 transition-all shadow-lg active:scale-95">
                    Send
                </button>
                ...
                {photos.map((photo, index) => {
                    ...
                    return (
                        <div key={photo.id} onClick={() => setLightboxIndex(index)} 
                            ...
                            {/* Selection Button */}
                            <button onClick={(e) => toggleSelect(photo, e)}
                                className={`absolute bottom-2 right-2 md:bottom-3 md:right-3 p-2 md:p-3 rounded-full shadow-lg transition-all active:scale-75 ${isSelected ? 'bg-yellow-400 text-black' : 'bg-white/20 text-white backdrop-blur-sm hover:bg-white hover:text-black'}`}>
      ...
      {lightboxIndex !== null && photos[lightboxIndex] && (
        <div className="fixed inset-0 z-[100] bg-black/98 backdrop-blur-sm flex items-center justify-center animate-scale-in" onClick={() => setLightboxIndex(null)}>
            <div className="absolute top-0 w-full p-4 flex justify-between items-center z-[102] bg-gradient-to-b from-black/80 to-transparent">
                <span className="text-white/70 text-xs font-mono">{photos[lightboxIndex].name}</span>
                <button onClick={() => setLightboxIndex(null)} className="p-2 bg-white/10 rounded-full text-white"><X size={24}/></button>
            </div>
            
            <button onClick={prevImage} className="absolute left-2 md:left-6 p-3 md:p-4 text-white/50 hover:text-white bg-black/20 hover:bg-white/10 rounded-full transition-all z-[102]"><ChevronLeft size={32} /></button>
            
            <div className="relative w-full h-full flex items-center justify-center p-2 md:p-10" onClick={(e) => e.stopPropagation()}>
                <img src={getHighResUrl(photos[lightboxIndex].thumbnailLink)} className="max-w-full max-h-full object-contain shadow-2xl rounded-sm" alt="Full" />
            </div>

            <button onClick={nextImage} className="absolute right-2 md:right-6 p-3 md:p-4 text-white/50 hover:text-white bg-black/20 hover:bg-white/10 rounded-full transition-all z-[102]"><ChevronRight size={32} /></button>
            
            {/* Bottom Action Bar in Lightbox */}
            <div className="absolute bottom-6 md:bottom-10 left-1/2 -translate-x-1/2 z-[102]" onClick={(e) => e.stopPropagation()}>
...
export default function App() {
  ...
  // Efek Preloader Website
  useEffect(() => {
    const timer = setTimeout(() => setInitialLoad(false), 2500); // Tampilkan preloader selama 2.5 detik
    return () => clearTimeout(timer);
  }, []);

  // Cek URL & Session LocalStorage
  useEffect(() => {
  ...
  return (
    <GoogleOAuthProvider clientId={GOOGLE_CLIENT_ID}>
      <GlobalStyles />
      <BackgroundSlideshow /> {/* <-- Latar Belakang Bergerak */}
      
      {initialLoad ? (
         <Preloader /> /* <-- Loading Awal yang Cantik */
      ) : (
